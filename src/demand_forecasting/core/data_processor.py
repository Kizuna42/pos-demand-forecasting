...
class DataProcessor:
    ...
    def detect_encoding(self, file_path: str) -> str:
        ...
        try:
            ...
            if confidence < 0.7:
                ...
                self.logger.warning(f"検出信頼度が低いため、デフォルトエンコーディングを使用: {encoding}")
    ...
    def remove_outliers(self, df: pd.DataFrame, method: str = 'iqr', columns: list = None) -> pd.DataFrame:
        """
        外れ値を除去する
    ...
    def create_basic_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if '金額' in df_features.columns and '数量' in df_features.columns:
                # ゼロ除算を避ける
                df_features['売上単価'] = df_features['金額'] / df_features['数量'].replace(0, np.nan)
    ...
    def _categorize_products(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 生鮮食品のカテゴリ定義
        category_keywords = {
            '野菜': ['キャベツ', 'レタス', 'トマト', 'きゅうり', '玉ねぎ', 'じゃがいも', 
                   'にんじん', 'ほうれん草', '白菜', 'もやし', 'ピーマン', 'なす'],
