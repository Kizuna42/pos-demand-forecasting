...
class DataProcessor:
    ...
    def remove_outliers_advanced(
        ...
        if method == "iqr_enhanced":
            ...
            if group_by_column and group_by_column in df_processed.columns:
                ...
                for group_name, group_data in df_processed.groupby(group_by_column):
                    group_outlier_mask = pd.Series([False] * len(group_data), index=group_data.index)
                    
                    for col in columns:
                        if col in group_data.columns and len(group_data) > 4:  # 最低5個のデータが必要
                            ...
                            if IQR > 0:  # IQRが0より大きい場合のみ実行
                                ...
                                col_outliers = (group_data[col] < lower_bound) | (group_data[col] > upper_bound)
                                group_outlier_mask |= col_outliers
                                
                                outlier_count = col_outliers.sum()
                                if outlier_count > 0:
                                    self.logger.info(f"{group_name} - {col}: {outlier_count} 件の外れ値を検出")
        ...
        self.logger.info(f"強化外れ値除去完了: 合計 {total_removed} 件を除去 ({removal_rate:.2f}%)")
        
        # 除去率が異常に高い場合は警告
        if removal_rate > 20:
            self.logger.warning(f"外れ値除去率が高すぎます ({removal_rate:.2f}%)。パラメータの見直しを推奨します。")
