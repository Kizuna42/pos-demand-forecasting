...
class ModelBuilder:
    ...
    def _perform_cross_validation(self, X: pd.DataFrame, y: pd.Series) -> Dict[str, Any]:
        ...
        self.logger.info(f"交差検証結果: R² = {cv_results['mean_score']:.4f} ± {cv_results['std_score']:.4f}")
    ...
    def calculate_shap_values(self, model: Any, X: pd.DataFrame, max_samples: int = 100) -> np.ndarray:
        """
        SHAP値を計算
