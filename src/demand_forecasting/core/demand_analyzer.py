...
class DemandCurveAnalyzer:
        ...
        try:
            ...
            if len(product_data) < 5:
                raise DemandAnalysisError(
                    f"データ不足: {product} のデータが{len(product_data)}件しかありません"
                )
            ...
            # 価格範囲を取得
            price_range = (price_demand_data["price"].min(), price_demand_data["price"].max())
    ...
    def _prepare_price_demand_data(self, product_data: pd.DataFrame) -> pd.DataFrame:
        ...
        price_demand = price_demand[(price_demand["price"] > 0) & (price_demand["quantity"] > 0)]
    ...
    def _apply_smoothing(self, data: pd.DataFrame, method: str = "savgol") -> pd.DataFrame:
        ...
        if method == "savgol":
            ...
            data_smooth["quantity"] = (
                data["quantity"].rolling(window=window_size, center=True, min_periods=1).mean()
            )
    ...
    def _fit_model(
        self, prices: np.ndarray, quantities: np.ndarray, model_func: Callable
    ) -> Tuple[Dict[str, float], float, Callable]:
        """モデルをフィッティング"""
        # パラメータの初期推定
        if model_func == self._linear_demand:
            initial_guess = [
                quantities.max(),
                (quantities.max() - quantities.min()) / (prices.max() - prices.min()),
            ]
        ...
        # curve_fitでパラメータを最適化
        popt, _ = curve_fit(model_func, prices, quantities, p0=initial_guess, maxfev=1000)
    ...
    def _calculate_optimal_price(self, demand_func: Callable, data: pd.DataFrame) -> float:
        ...
        try:
            ...
            result = minimize_scalar(
                negative_revenue, bounds=(price_min, price_max), method="bounded"
            )
    ...
    def analyze_multiple_products(
        self, df: pd.DataFrame, products: list = None
