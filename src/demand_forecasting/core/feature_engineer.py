...
class FeatureEngineer:
    ...
    def _add_holiday_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_holidays['祝日フラグ'] = df_holidays['年月日'].dt.date.isin(holiday_dates.date).astype(int)
        ...
        if '月初フラグ' in df_holidays.columns and '月末フラグ' in df_holidays.columns:
            ...
            df_holidays['セール期間フラグ'] = (
                (df_holidays['年月日'].dt.day <= 5) | 
                (df_holidays['年月日'].dt.day >= 25)
            ).astype(int)
    ...
    def integrate_weather_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            if not self.feature_config.get('weather_features', {}).get('enable_weather_api', False):
    ...
    def _add_synthetic_weather_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        def synthetic_temperature(month):
            temp_map = {
                1: 5, 2: 7, 3: 12, 4: 18, 5: 23, 6: 26,
                7: 29, 8: 31, 9: 27, 10: 21, 11: 15, 12: 8
            }
        ...
        df_synthetic['最低気温'] = df_synthetic['平均気温'] - np.random.uniform(3, 8, len(df_synthetic))
        df_synthetic['最高気温'] = df_synthetic['平均気温'] + np.random.uniform(3, 8, len(df_synthetic))
    ...
    def _add_weather_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_weather_features['気温_移動平均'] = df_weather_features['平均気温'].rolling(
    ...
    def _remove_highly_correlated_features(self, df: pd.DataFrame, features: List[str], threshold: float = 0.8) -> List[str]:
        """高相関の特徴量ペアを除去"""
