...
class QualityEvaluator:
    ...
    def _calculate_overall_success_rate(self, quality_distribution: Dict[str, int]) -> float:
        ...
        success_count = quality_distribution.get('Premium', 0) + quality_distribution.get('Standard', 0)
    ...
    def evaluate_model_reliability(self, model_metrics: Dict[str, float]) -> Dict[str, Any]:
        ...
        reliability_indicators = {
            ...
            'overfitting_risk': '高' if overfitting_score > 0.2 else '低' if overfitting_score < 0.1 else '中',
            'performance_consistency': abs(r2_score - cv_mean) < 0.05
        }
        
        # 総合信頼性スコア
        reliability_score = self._calculate_reliability_score(reliability_indicators, model_metrics)
    ...
    def _calculate_reliability_score(self, indicators: Dict[str, Any], metrics: Dict[str, float]) -> float:
        """信頼性スコアを計算"""
        score = 0.0
    ...
    def _generate_recommendations(self, indicators: Dict[str, Any]) -> List[str]:
        ...
        if not indicators['performance_consistency']:
            recommendations.append("モデルの一貫性向上のため、ハイパーパラメータの調整を行ってください")
    ...
    def _identify_improvement_priorities(self, detailed_analysis: List[Dict]) -> List[str]:
        ...
        # Rejectedが多い場合
        rejected_count = sum(1 for item in detailed_analysis if item['quality_level'] == 'Rejected')
