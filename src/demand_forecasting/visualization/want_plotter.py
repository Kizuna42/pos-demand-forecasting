...
class WantPlotter:
    ...
    def _setup_style(self):
        ...
        sns.set_palette([self.want_colors['primary'], self.want_colors['secondary'], 
                        self.want_colors['accent'], self.want_colors['success']])
    ...
    def create_demand_curve_plot(self, demand_results: Dict[str, Any], save_path: Optional[str] = None) -> str:
        """
        需要曲線プロットを作成
        ...
        try:
            ...
            # 左側: 需要曲線
            self._plot_demand_curve(ax1, price_demand_data, demand_func, optimal_price, current_price)
            ax1.set_title(f"{demand_results['product_name']} - 需要曲線", fontweight='bold')
            
            # 右側: 収益曲線
            self._plot_revenue_curve(ax2, demand_func, optimal_price, current_price, 
                                   demand_results['price_range'])
            ...
            plt.savefig(save_path, dpi=self.viz_config.get('dpi', 300), 
    ...
    def _plot_demand_curve(self, ax, price_demand_data, demand_func, optimal_price, current_price):
        """需要曲線をプロット"""
        # 実データ点をプロット
        ax.scatter(price_demand_data['price'], price_demand_data['quantity'], 
                  color=self.want_colors['secondary'], alpha=0.6, s=50, label='実データ')
        
        # 需要曲線をプロット
        price_range = np.linspace(price_demand_data['price'].min(), 
                                 price_demand_data['price'].max(), 100)
        demand_curve = [max(0, demand_func(p)) for p in price_range]
        
        ax.plot(price_range, demand_curve, color=self.want_colors['primary'], 
               linewidth=3, label='需要曲線')
        
        # 最適価格と現在価格を表示
        if optimal_price:
            optimal_quantity = max(0, demand_func(optimal_price))
            ax.axvline(optimal_price, color=self.want_colors['success'], 
                      linestyle='--', alpha=0.8, label=f'最適価格: ¥{optimal_price:.0f}')
            ax.scatter([optimal_price], [optimal_quantity], 
                      color=self.want_colors['success'], s=100, zorder=5)
        
        if current_price:
            current_quantity = max(0, demand_func(current_price))
            ax.axvline(current_price, color=self.want_colors['warning'], 
                      linestyle='--', alpha=0.8, label=f'現在価格: ¥{current_price:.0f}')
            ax.scatter([current_price], [current_quantity], 
                      color=self.want_colors['warning'], s=100, zorder=5)
    ...
    def _plot_revenue_curve(self, ax, demand_func, optimal_price, current_price, price_range):
        ...
        if optimal_price:
            ...
            ax.axvline(optimal_price, color=self.want_colors['success'], 
                      linestyle='--', alpha=0.8, label=f'最適価格: ¥{optimal_price:.0f}')
            ax.scatter([optimal_price], [optimal_revenue], 
                      color=self.want_colors['success'], s=100, zorder=5)
        ...
        if current_price:
            ...
            ax.axvline(current_price, color=self.want_colors['warning'], 
                      linestyle='--', alpha=0.8, label=f'現在価格: ¥{current_price:.0f}')
            ax.scatter([current_price], [current_revenue], 
                      color=self.want_colors['warning'], s=100, zorder=5)
    ...
    def create_feature_importance_plot(self, feature_importance: Dict[str, float], 
                                     product_name: str, save_path: Optional[str] = None) -> str:
        """
        特徴量重要度プロットを作成
    ...
    def create_quality_dashboard(self, quality_data: Dict[str, Any], 
        ...
        try:
            ...
            # 実用化準備状況分布
            self._plot_implementation_distribution(ax2, quality_data.get('implementation_distribution', {}))
    ...
    def _plot_quality_distribution(self, ax, quality_dist):
        ...
        colors = [self.want_colors['success'], self.want_colors['accent'], 
                 self.want_colors['warning'], self.want_colors['error']][:len(levels)]
        
        wedges, texts, autotexts = ax.pie(counts, labels=levels, colors=colors, autopct='%1.1f%%', 
                                         startangle=90)
    ...
    def _plot_r2_distribution(self, ax, quality_data):
        ...
        # 統計値を表示
        ax.text(0.5, 0.7, f'平均R²スコア', ha='center', va='center', 
               transform=ax.transAxes, fontsize=14, fontweight='bold')
        ax.text(0.5, 0.5, f'{avg_r2:.3f}', ha='center', va='center', 
               transform=ax.transAxes, fontsize=24, fontweight='bold', 
               color=self.want_colors['primary'])
        ax.text(0.5, 0.3, f'標準偏差: {r2_std:.3f}', ha='center', va='center', 
    ...
    def create_comprehensive_report_plots(self, analysis_results: List[Dict[str, Any]], 
            ...
            for result in analysis_results:
                if 'demand_results' in result:
                    demand_plot_path = self.create_demand_curve_plot(
                        ...
                        save_dir / f"demand_{result['product_name']}.png"
                    )
