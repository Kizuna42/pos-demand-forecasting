...
class DemandForecastingPipeline:
    ...
    def _validate_time_series_quality(self, product_data: pd.DataFrame, product_name: str) -> bool:
        ...
        try:
            ...
            date_range = (product_data_sorted['年月日'].max() - product_data_sorted['年月日'].min()).days
            ...
            if len(product_data) < expected_records:
                self.logger.warning(f"{product_name}: データ密度が低すぎます (期間:{date_range}日, レコード数:{len(product_data)})")
                return False
            ...
            if 'month' in product_data.columns or '月' in product_data.columns:
                ...
                # 月次売上の変動係数（CV）をチェック
                cv = monthly_sales.std() / monthly_sales.mean() if monthly_sales.mean() > 0 else float('inf')
                if cv > 2.0:  # 変動係数が2.0を超える場合は不安定すぎる
                    self.logger.warning(f"{product_name}: 月次売上の不安定性が高すぎます (CV: {cv:.2f})")
                    return False
            ...
            if zero_sales_ratio > 0.5:  # ゼロ売上が50%を超える場合
                self.logger.warning(f"{product_name}: ゼロ売上の比率が高すぎます ({zero_sales_ratio:.1%})")
                return False
