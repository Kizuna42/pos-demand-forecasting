...
"""
    ...
    def sample_data_file(self, temp_dir):
        ...
        products = ['りんご', 'キャベツ', '牛肉', 'まぐろ', 'じゃがいも', 'にんじん', '豚肉', 'さば']
        ...
        for date in dates:
            for product in np.random.choice(products, size=np.random.randint(2, 6), replace=False):
                # より現実的な価格・需要関係
                base_prices = {
                    'りんご': 120, 'キャベツ': 80, '牛肉': 600, 'まぐろ': 400,
                    'じゃがいも': 60, 'にんじん': 70, '豚肉': 300, 'さば': 200
                }
                
                base_quantities = {
                    'りんご': 25, 'キャベツ': 20, '牛肉': 8, 'まぐろ': 12,
                    'じゃがいも': 30, 'にんじん': 22, '豚肉': 15, 'さば': 18
                }
        ...
        try:
            ...
            for product in target_products:
                if product in analyzed_products:
                    # 商品が分析されている場合の検証
                    product_result = next(r for r in analysis_results if r['product_name'] == product)
    ...
    def test_pipeline_performance_with_large_dataset(self, sample_config_file, temp_dir):
        ...
        try:
            ...
            data = []
            for date in dates:
                for product in np.random.choice(products, size=np.random.randint(5, 15), replace=False):
